...
int BI_set_str(BIGINT rop, const char* str, int base) {
	...
	BI_set_ui(op1, 1);
	if ((op2 = (struct bigint_st *)malloc(sizeof(struct bigint_st))) == NULL) {
		printf("error in allocate memory for op2\n");
	}
	if ((op3 = (struct bigint_st *)malloc(sizeof(struct bigint_st))) == NULL) {
		printf("error in allocate memory for op3\n");
	}
	...
	for (i=MAXSIZE; i>=0; i--) {
		if (op2->value[i] != 0) {
			...
			for (j=MAXSIZE; j>=0; j--) {
				...
				tmp2->value[i] = (uint8_t)rem;
			}
			tmp2->signbit = op1->signbit * op2->signbit;
...
int main() {
	...
	printf("size of n: %u\n", n->size);
	// BI_randb(n, k);
	// printf("random number n (%u bits):\n", k);
	// for (i=0; i<=MAXSIZE; i++) {
	// 	printf("%d: %u\n", i, n->value[i]);
	// }
	// printf("size of n: %u\n", n->size);
	//BI_get_str(test, 10, n);
	//printf("%s\n", test);
	free(n);
	return 0;
}